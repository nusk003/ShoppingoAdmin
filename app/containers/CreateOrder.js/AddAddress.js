...
class AddAddress extends Component {
  ...
  componentDidMount(){
      this.props.init({phone:this.props.phone})

      this.backListener = history.listen(location => {
        if (location.action === "POP") {
          // Do your stuff
          this.props.closeAddAddressModal()
        }
      });
  }
  ...
  handleChange = e => {
    this.props.init({[e.target.name]:e.target.value})
  }
          ...
          <Grid item xs={12} md={6}>
            <Paper className={classes.root}>
              ...
              <Typography component="p">
                You can create new address for customer here
              </Typography>
...
const mapDispatchToProps = dispatch => ({
  ...
  closeModal : () =>dispatch(actions.closeAddAddressModal())
});
...
const FormInit = connect(
  state => ({
    ...
    phone : state.get('customer').phone
  }),
  mapDispatchToProps,
)(ReduxFormMapped);
